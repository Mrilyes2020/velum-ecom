...
function LiveSocialProof() {
  ...
  return (
    <>
      <div
        className={`fixed bottom-4 right-4 z-40 max-w-[280px] rounded-xl bg-card border border-gold/40 shadow-xl px-4 py-3 ${show ? "slide-in-right" : "opacity-0"}`}
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-sm">
            <div className="font-bold">{p.name} من {p.city}</div>
            <div className="text-xs text-muted-foreground">طلب VELUM للتو</div>
          </div>
        </div>
      </div>
